...
async def get_user_credit_dict(session, user_id_list: List[str]) -> Dict[str, int]:
    ...
    for user_id in user_id_list:
        ...
        tasks.append(
            asyncio.ensure_future(get_and_get_field(session, url, "credit", user_id))
        )
    user_id_credit: List[Tuple[str, int]] = list(await asyncio.gather(*tasks))
    return dict(user_id_credit)


async def get_item_stock_dict(
    session, item_id_list: Union[List[str], str]
) -> Dict[str, int]:
    tasks = []
    ...
    for item_id in item_id_list:
        ...
        tasks.append(
            asyncio.ensure_future(get_and_get_field(session, url, "stock", item_id))
        )
...
def parse_log(tmp_dir, prior_user_state: Dict[str, int]):
    ...
    logger.info(
        f"Stock service inconsistencies in the logs: {i - (NUMBER_OF_ITEMS * ITEM_STARTING_STOCK)}"
    )
...
async def verify_systems_consistency(tmp_dir: str, item_ids, user_ids):
    ...
    server_side_items_bought: int = (NUMBER_OF_ITEMS * ITEM_STARTING_STOCK) - list(
        iis.values()
    )[0]
    logger.info(
        f"Stock service inconsistencies in the database: "
        f"{server_side_items_bought - (NUMBER_OF_ITEMS * ITEM_STARTING_STOCK)}"
    )
    logged_user_credit: int = sum(pus.values())
    logger.info(
        f"Payment service inconsistencies in the logs: {abs(CORRECT_USER_STATE - logged_user_credit)}"
    )
    server_side_user_credit: int = sum(list(uic.values()))
    logger.info(
        f"Payment service inconsistencies in the database: {abs(CORRECT_USER_STATE - server_side_user_credit)}"
    )
